...
namespace PE.Plugins.Network.Droid
{
    public class RestService : IRestService
    {
        ...
        public RestService(NetworkConfiguration configuration)
        {
            ...
            //  refresh the token
            if ((configuration.RefreshTokenCallback != null) || (configuration.RefreshTokenTime > 0))
            {
                RefreshToken = configuration.RefreshTokenCallback;
                RefreshTokenTime = configuration.RefreshTokenTime * 1000;
        ...
        public async Task<bool> PingAsync()
        {
            ...
            {
                //  ping the API
                System.Diagnostics.Debug.WriteLine(string.Format("RestService: Pinging the API...{0}", _Configuration.BaseUrl));
        ...
        public async Task DeleteAsync(string address)
        {
            ...
            {
                AddToken(client);
                var response = await client.DeleteAsync(new Uri(string.Format("{0}/{1}", _Configuration.BaseUrl, address)));
        ...
        {
            System.Diagnostics.Debug.WriteLine(string.Format("*** RestService - GET 1: {0}", address));
            using (var client = new HttpClient())
            {
                AddToken(client);
                var response = await client.GetAsync(new Uri(string.Format("{0}/{1}", _Configuration.BaseUrl, address)));
                return await GetResponse<TReturn>(response);
            }
        }
        ...
        {
            System.Diagnostics.Debug.WriteLine(string.Format("*** RestService - GET 2: {0} for {1}", address, returnType.Name));
            using (var client = new HttpClient())
            {
                AddToken(client);
                var response = await client.GetAsync(new Uri(string.Format("{0}/{1}", _Configuration.BaseUrl, address)));
        ...
        {
            System.Diagnostics.Debug.WriteLine(string.Format("*** RestService - GET 3: {0}", address));
            using (var client = new HttpClient())
            {
                AddToken(client);
        ...
        public async Task PostAsync(string address)
        {
            ...
            {
                AddToken(client);
                var response = await client.PostAsync(new Uri(string.Format("{0}/{1}", _Configuration.BaseUrl, address)), new StringContent(string.Empty, Encoding.UTF8, Constants.CONTENT_TYPE));
                CheckResponse(response);
            }
        }
        ...
        public async Task PostAsync<TEntity>(string address, TEntity entity)
        {
            using (var client = new HttpClient())
            {
                ...
                var response = await client.PostAsync(new Uri(string.Format("{0}/{1}", _Configuration.BaseUrl, address)), new StringContent(payload, Encoding.UTF8, Constants.CONTENT_TYPE));
        ...
        public async Task PutAsync(string address)
        {
            ...
            {
                AddToken(client);
                var response = await client.PutAsync(new Uri(string.Format("{0}/{1}", _Configuration.BaseUrl, address)), new StringContent(string.Empty, Encoding.UTF8, Constants.CONTENT_TYPE));
                CheckResponse(response);
            }
        }
        ...
        public async Task PutAsync<TEntity>(string address, TEntity entity)
        {
            using (var client = new HttpClient())
            {
                AddToken(client);
                var payload = Serializer.Serialize(entity);
                var response = await client.PutAsync(new Uri(string.Format("{0}/{1}", _Configuration.BaseUrl, address)), new StringContent(payload, Encoding.UTF8, Constants.CONTENT_TYPE));
        ...
        private async Task<T> GetResponse<T>(HttpResponseMessage response)
        {
            ...
            if (response.Headers.TryGetValues(_Configuration.AuthToken, out tokens))
            {
                if (!string.IsNullOrEmpty(tokens.First()))
                {
                    ...
                    _LocalStorage.Put(_Configuration.AuthToken, tokens.First());
                }
            }
        ...
        private async Task<object> GetResponse(Type returnType, HttpResponseMessage response)
        {
            ...
            if (response.Headers.TryGetValues(_Configuration.AuthToken, out tokens))
            {
                ...
                {
                    //  save in local storage
                    //await _LocalStorage.PutAsync(_NetworkManager.AuthToken, tokens.First());
                    _LocalStorage.Put(_Configuration.AuthToken, tokens.First());
                }
            }
        ...
        protected void AddToken(HttpClient client)
        {
            ...
            //string token = await _LocalStorage.GetAsync<string>(_NetworkManager.AuthToken);
