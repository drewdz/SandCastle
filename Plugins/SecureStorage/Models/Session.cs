...
namespace PE.Plugins.SecureStorage.Models
{
	...
	public class Session
	{
		...
		{
			var validPassword = CryptoService.VerifyString(password, HashedPassword, passwordSalt);
		...
		byte[] HashPassword(string password)
        {
            if (passwordSalt == null)
                passwordSalt = CryptoService.GenerateSalt();

            return CryptoService.HashString(password, passwordSalt);
        }
		#endregion Methods
	}
}
